...
if (mysqli_num_rows($get_data) > 0) {
    ...
    // Fetch the company name from the user data
    $company_name = $user['name'];

    $find_job = mysqli_query($connect, "SELECT * FROM tb_add_job WHERE company_name = '$company_name'");
    $modifiedData = [];
                    ...
                    <tbody>
                      <?php foreach ($modifiedData as $row) : ?>
                        <tr class="fs-3">
                          ...
                          <td>
                            <button class="btn btn-warning btn-sm mb-2" onclick="window.location='<?php echo 'edit_job.php?job_title=' . urlencode($row['job_title']) . '&company_name=' . urlencode($row['company_name']) . '&company_location=' . urlencode($row['company_location']); ?>'">
                              <i class="ti ti-pencil me-2"></i> Edit
                            </button>
                            <button class="btn btn-danger btn-sm mb-2" onclick="window.location='<?php echo 'hapus.php?job_title=' . urlencode($row['job_title']) . '&company_name=' . urlencode($row['company_name']) . '&company_location=' . urlencode($row['company_location']); ?>'">
